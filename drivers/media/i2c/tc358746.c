...
static inline struct tc358746_state *
notifier_to_tc358746(struct v4l2_async_notifier *n)
{
       return container_of(n, struct tc358746_state, notifier);
}
...
static int tc358746_notify_bound(struct v4l2_async_notifier *notifier,
                                 ...
                                 struct v4l2_async_subdev *asd)
{
       struct tc358746_state *state = notifier_to_tc358746(notifier);
 
       return v4l2_create_fwnode_links(sd, &state->sd);
 }
...
static int tc358746_async_register(struct tc358746_state *state)
{
	...
	//TODO probably remove beolow!
	// v4l2_async_notifier_init(&state->notifier);

	// ep = fwnode_graph_get_endpoint_by_id(dev_fwnode(state->sd.dev),
	// 			port, 0,
	...
	// 		return -ENOMEM;
	// 	}
	// 	//TODO, The notifer will look for a match for a FWN node, before we are registered.
	// 	//But at this point we do not have an endpoint. 
	...
	// 	}
	// }
	// dmo_print("tc358746 %s %d", __func__, ep != NULL);
