...
struct i2c_string_data {
    uint8_t reg;        // Start register
    uint8_t length;     // Length of string (max 32 bytes)
...
static const struct of_device_id lvibattery_of_match[] = {
    { .compatible = "lvibattery", },
    {},
};
MODULE_DEVICE_TABLE(of, lvibattery_of_match);

static long lvibattery_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
    struct i2c_data reg_data;
    struct i2c_string_data string_data;
    int ret;
    struct i2c_msg msgs[2];
    uint8_t reg;
    uint8_t buffer[2];  // Buffer for 16-bit values (2 bytes)
    ...
    switch (cmd) {
            ...
            if (copy_from_user(&reg_data, (struct i2c_data __user *)arg, sizeof(reg_data))) {
            ...
            msgs[0].addr  = 0x0b;  // Smart battery address
            ...
            msgs[1].addr  = 0x0b;  // Smart battery address
            ...
            pr_info("Data read from I2C device: reg=0x%x, value=0x%x\n", reg, reg_data.value);

            if (copy_to_user((struct i2c_data __user *)arg, &reg_data, sizeof(reg_data))) {
        ...
        case IOCTL_WRITE_DATA:
            ...
            pr_info("Writing to I2C device: reg=0x%x, value=0x%x\n", reg_data.reg, reg_data.value);

            /* Send the register address and the data to write */
            msgs[0].addr  = 0x0b;  // Smart battery address
            msgs[0].flags = 0;     // Write
            msgs[0].len   = 2;     // Register + data
            uint8_t buffer[2] = { reg_data.reg, reg_data.value };
            msgs[0].buf   = buffer;
        ...
        case IOCTL_READ_STRING:
            pr_info("IOCTL_READ_STRING command\n");

            if (copy_from_user(&string_data, (struct i2c_string_data __user *)arg, sizeof(string_data))) {
                pr_err("Failed to copy string data from user space\n");
                return -EFAULT;
            }

            if (string_data.length > MAX_STRING_SIZE) {
                pr_err("Requested length exceeds maximum allowed size\n");
                return -EINVAL;
            }
            ...
            ret = i2c_transfer(i2c_client->adapter, msgs, 2);
            if (ret < 0) {
                pr_err("Failed to read string from I2C device: %d\n", ret);
                return ret;
            }

            pr_info("String read from I2C device: reg=0x%x, length=%d\n", reg, string_data.length);

            if (copy_to_user((struct i2c_string_data __user *)arg, &string_data, sizeof(string_data))) {
                pr_err("Failed to copy string data to user space\n");
                return -EFAULT;
            }
            return 0;
...
static int lvibattery_open(struct inode *inode, struct file *file) {
    pr_info("lvibattery device opened\n");
    return 0;
}

static int lvibattery_release(struct inode *inode, struct file *file) {
    pr_info("lvibattery device released\n");
    return 0;
}
...
static int lvibattery_probe(struct i2c_client *client, const struct i2c_device_id *id) {
    pr_info("Probing lvibattery driver\n");
    ...
    if (IS_ERR(device_create(lvibattery_class, NULL, MKDEV(major, 0), NULL, DEVICE_NAME))) {
...
static int lvibattery_remove(struct i2c_client *client) {
    pr_info("Removing lvibattery driver\n");

    device_destroy(lvibattery_class, MKDEV(major, 0));
    class_destroy(lvibattery_class);
    unregister_chrdev(major, DEVICE_NAME);

    pr_info("lvibattery driver removed\n");
...
module_i2c_driver(lvibattery_driver);  // Registers and unregisters the driver automatically

MODULE_LICENSE("GPL");
MODULE_AUTHOR("Max Borglowe");
MODULE_DESCRIPTION("I2C Kernel Module for Smart Battery at 0x0b with DT support");
