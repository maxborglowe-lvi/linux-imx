			    ...
			    unsigned long arg)
{
	...
	if (reg_size == REG_SIZE_8) {
		...
		addr_sub =
			data.reg % EEPROM_CELL_SIZE; // Calculate the subaddress
	}
	...
	case IOCTL_WRITE_DATA:
		...
		case REG_SIZE_8:
			...
			printk(KERN_INFO
			       "EEPROM: Writing to reg 0x%02X data 0x%02X\n",
			       addr_sub, data.data);
			buf8[0] = addr_sub; // 8-bit register address
		...
		case REG_SIZE_16:
			printk(KERN_INFO
			       "EEPROM: Writing to reg 0x%04X data 0x%02X\n",
			       data.reg, data.data);
			buf16[0] = (data.reg >> 8) &
		...
		case REG_SIZE_32:
			printk(KERN_INFO
			       "EEPROM: Writing to reg 0x%08X data 0x%02X\n",
			       data.reg, data.data);
			buf32[0] = (data.reg >> 24) &
		...
		msgs[0].addr = i2c_client->addr + addr_cell;
		msgs[0].flags = 0;

		ret = i2c_transfer(i2c_client->adapter, msgs, 1);
		if (ret < 0) {
			printk(KERN_ERR
			       "EEPROM: Failed to write to the EEPROM\n");
			return -EIO;
		}
	...
	case IOCTL_READ_DATA:
		if (reg_size == REG_SIZE_8) {
			...
			msgs[1].addr = i2c_client->addr + addr_cell;
		...
		if (ret < 0) {
			printk(KERN_ERR
			       "EEPROM: Failed to read from the EEPROM\n");
			return -EIO;
		}
